...
class Dashing.Rickshawgraph extends Dashing.Widget
  ...
  # Take a float like 2.3434343535 and turn it into "2.3"
  formatNumber = (number) ->
  ...
  ready: ->
    ...
    # get data-cool value
    @cool = @get('cool') if @get('cool')
    # get data-warm value
    @warm = @get('warm') if @get('warm')
  ...
  onData: (data) ->
    # customization on color changing base on status
    $node = $(@node)
    ...
    # end of customization on colors
